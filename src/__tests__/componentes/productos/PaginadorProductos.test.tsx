...
describe("PaginadorProductos Test", () => {
  ...
  it("Renderizar con  20 productos debe pintar 2 botones", () => {
    ...
    expect(componentWrapper.find('[data-testid="boton-paginar0"]').text()).toBe(
      "1"
    );
    expect(componentWrapper.find('[data-testid="boton-paginar1"]').text()).toBe(
      "2"
    );
    expect(
      componentWrapper.find('[data-testid="boton-paginar2"]').exists()
    ).toBeFalsy();
  });
});
